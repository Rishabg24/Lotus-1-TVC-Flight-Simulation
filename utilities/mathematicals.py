...
class Motor:
    ...
    def compute_mass_burned(self):
        ...
        mdot_array = thrust_array / (
            self.specific_impulse * 9.81
        )  # mdot = F(t) / (I_sp * g_0)

        # integrate mdot from 0 to t to get mass burned
        mass_burned = cumulative_trapezoid(
            mdot_array,
            x=self.time_data[self.time_data <= self.time_data[-1]],
            initial=0,
        )
        return mass_burned

    def compute_mass_propellant(
        self,
        t,
    ):
        if np.isscalar(t):
...
class Rocket:
    ...
    def compute_dist_to_COM(self, t, motor):
        ...
        if np.isscalar(t):
            ...
            X_COG = np.where(
                total_mass > 0,
                (dry_mass * DRY_CG + prop_mass * PROP_CG) / total_mass,
                0.0,
            )
            return X_COG

    def compute_inertia(self, t, motor, X_CG=None):
...
def dynamics(t, x, x_measured, K, g, motor=None, rocket=None, phase="powered"):
    global saturation_counter
    theta, theta_dot, v, h = x  # true state — used for physics
    measured_attitude = x_measured[:2]  # noisy state — used only for control law

    u = float((-K @ measured_attitude).item())
    if np.abs(u) > np.deg2rad(20):
...
def coasting_dynamics(t, x, x_measured, K_alpha, K_d, g, theta_wind, inertia):
    """
...
def RK4(f, t, x, h, x_measured, *args, **kwargs):
    k1 = f(t, x, x_measured, *args, **kwargs)
    k2 = f(t + h / 2, x + h / 2 * k1, x_measured, *args, **kwargs)
    k3 = f(t + h / 2, x + h / 2 * k2, x_measured, *args, **kwargs)
    k4 = f(t + h, x + h * k3, x_measured, *args, **kwargs)
